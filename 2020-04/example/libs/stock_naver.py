...
def get_종목별_일별_거래량(code, max_page=3):
    ...
    for page in range(1, max_page+1):  # 페이지당 20개 * 3페이지
        ...
        res = requests.get("https://finance.naver.com/item/frgn.nhn", params=params)
        html = res.text
...
def get_kospi200(code, max_page=1):
    ...
    for page in range(1, max_page+1):
        ...
        res = requests.get("https://finance.naver.com/sise/sise_index_day.nhn", params=params)
        res.raise_for_status()
        html = res.text
